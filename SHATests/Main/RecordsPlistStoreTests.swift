...
class RecordsPlistStoreTests: XCTestCase {
    ...
    func readMockPlist() -> [Record] {
        let recordsPlist = (NSArray(contentsOfFile: sut.plistPath) ?? []) as! [PlistDictionary]
        return recordsPlist.map(Record.init)
    }
    ...
    func testUpdateShouldInsertNonexistentRecord() {
        ...
        // Then
        wait(for: [expect], timeout: 0.6)
        let actualRecords = readMockPlist()
        XCTAssertEqual(actualRecords, [testRecords[0], newRecord] , "Store should add new record")
    }
    ...
    func testUpdateShouldUpdateTheRecordInRecords() {
        ...
        wait(for: [expect], timeout: 0.6)
        let actualRecords = readMockPlist()
        XCTAssertEqual(actualRecords, [testRecords[0], newRecord, testRecords[2]], "Store should store updated record")
    }
}
